...
if  __name__ == "__main__":
    ...
    checkPath = "CheckPoints/seq/best.pth"
    ...
    for ax in axs.flat:
        ...
        # cbar = plt.colorbar()
        # cbar.set_label("mg/l")
    plt.savefig('./training_log_Seq_'+ str(num_epochs) + '_' + str(layer_num) + '_' + str(loaderoffset) + '.jpg')
    plt.show()
    print("total test time:" , time.time()-time_start)
    with open('training_log_Seq_'+ str(num_epochs) + '_' + str(layer_num) + '_' + str(loaderoffset) + '.txt', 'a') as f:
